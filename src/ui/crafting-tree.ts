...
type RenderCraftingTreeOptions = {
  ...
  searchText: string
  recipes: Recipe[]
  onRecipeChoiceChanged: (material: string, recipeIndex: number) => void
}
...
export function renderCraftingTree({
  ...
  nodeSelection
    ...
    .attr('height', (node) => {
      ...
      return getRecipeChoicesCount(recipes, node.data.material) > 1 ? 115 : 84
    })
    .attr('rx', 10)
    .attr('ry', 10)
  ...
  nodeSelection
    ...
    .on('change', function (event, node) {
      ...
      onRecipeChoiceChanged(node.data.material, Number(select.value))
    })
    .each(function (node) {
      const select = d3.select(this)
      const choices = recipesProducing(recipes, node.data.material)
      const selectedIndex = recipeChoices.get(node.data.material) ?? 0
      ...
      select
        ...
        .property('selected', (_, index) => index === selectedIndex)
        .text((recipe) => `${recipe.name} — ${recipe.duration} min`)
    })
}
...
function recipesProducing(recipes: Recipe[], material: string): Recipe[] {
  return recipes.filter((recipe) => recipe.outputs.some((output) => output.material === material))
}
